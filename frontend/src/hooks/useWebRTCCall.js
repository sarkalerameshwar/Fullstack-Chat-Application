...
export function useWebRTCCall(selectedUser) {
  ...
  const cleanup = useCallback(() => { clearInterval(timer.current); peer.current?.close(); peer.current = null; localStream.current?.getTracks().forEach((t) => t.stop()); localStream.current = null; setLocalMedia(null); setRemoteMedia(null); setCall(null); setSeconds(0); callId.current = null; }, []);
